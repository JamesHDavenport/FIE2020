...
The academic skill sets that a good Chief Information Security Officer (CISO) should have, we can be break them into three rough groupings.
\begin{itemize}
...
This Cybersecurity module is part of a general computing degree. As such there is a wide range of backgrounds on the course. In section \ref{sec:academic} the question ``how much cryptography need a security expert know'' was posed. The answer for this course is ``about 90 minutes worth'', which seems to cover the bases well enough: for example enough time to enable  cryptographic hashing to underpin the lecture on password management.
...
\end{enumerate}
Question \ref{DNS} is meant to help students realise the interconnected nature of today's Internet, and the hidden dependencies. A lecture related to DNS, and in particular the recent wave of home router attacks  \cite[and others]{Balaji2020a} was included.
\par
...
\subsection{Computer Science - embedding cybersecurity accross the curricula}\label{sec:fifth} % TP
...
As can be seen from {\ref{tab:CaseStudy5}}, cybersecurity is integrated across the syllabus. The  large cohort of students studying the programme has quite a diverse range of interests. 
...
The team project based nature of both these modules has been adopted to facilitate the development of the students Human Skills \ref{sec:human}. As in \ref{sec:second}, this year Covid-19 and the consequent dispersal of the students has meant that they are also learning remote team working: an unexpected bonus. Appropriate academic skills \ref{sec:academic} are evidenced partly in the manner the practical work is completed but also in the case of the final year Team Project and Professionalism module by the quality and content of the evaluative report.
...
In the previous section, we seen some examples in which the delivery of cybersecurity has been enhanced. The approaches emphasize the need for both the academic and human skills. 
...
\item Cybersecurity can be seen to be enthusing and exciting potential students, this represents an opportunity to potentially extend and diversify the student basis as well as an opportunity to promote knowledge of cybersecurity issues in the wider community.
